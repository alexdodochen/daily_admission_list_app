---
name: subtable H 註記 → N-V R 備註 sync
description: 子表格 H「註記」非空時必須同步到 N-V 區的 R「備註」欄;H 空則保留 R 手填值
type: feedback
---

`ordering_service.integrate_ordering` 與 `sync_ordering_after_diff` 在寫
N-V 入院序時,R 欄(備註)取值規則:
...
- 子表格 H 是醫師端的真實意圖(例:不排導管、待會診);R 欄如果不同步,
  N-V 區看不到關鍵 flag,Step 5 cathlab keyin 與 LINE 推播會缺資訊
...
- Q(備註住服)與 V(改期)規則不同 ── 那兩個永遠保留,不從子表格同步
- `sync_ordering_after_diff` 新增 patient(沒有 existing row)的情況,R 直接
  寫 `info.get("note", "")`,因為沒有手填值可保留
- Phase 18 / 2026-05-20:使用者明示「within the N-V ordering area 沒錯就是那邊」
