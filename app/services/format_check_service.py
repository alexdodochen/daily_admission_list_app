...
Ported from the admission-format-check skill. Minimum viable scope:
  ...
  * N-U header is the canonical 8-col ordering layout
  ...
  * 病歷號 columns (main I / N-U S / sub B) are TEXT format so leading zeros stick
...
Fixable by this service:
  ...
  - order_header_wrong       → rewrite N1:U1 (8-col layout)
...
def check(date: str) -> dict:
    ...
    order_row = sheet_service.read_range(ws, "N1:U1")
    order_header = order_row[0] if order_row else []
    order_header = (order_header + [""] * 8)[:8]
...
def fix(date: str, types: Optional[list[str]] = None) -> dict:
    ...
    if any(i["type"] == "order_header_wrong" for i in issues) and want("order_header_wrong"):
        sheet_service.write_range(ws, "N1:U1", [EXPECTED_ORDER_HEADER], raw=False)
